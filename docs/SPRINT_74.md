...
## Verified identity to local session bridge

A cryptographically verified OIDC issuer/subject can now cross into the existing local session model only through an explicit external-identity link. Unknown subjects fail closed; email, domain, tenant, role, or other claims are not used for implicit matching or provisioning. The linked local user must still exist and be enabled. Session creation delegates to the existing `AuthStore.create_session()` path, preserving the same expiry, home-tenant membership requirement, and active-tenant initialization used by password login. A missing home-tenant membership or otherwise unavailable local account fails closed. This bridge does not itself expose browser routes; callback orchestration remains a separate final integration boundary.

## Browser login and callback orchestration

The isolated OIDC layers are now composed into public login and callback routes. OIDC remains optional; when configured, `SECSCAN_OIDC_REDIRECT_URI` is also required and must be an explicit absolute HTTPS URL, preventing callback construction from untrusted Host or proxy headers. Login performs bounded discovery, creates a one-time transaction, and redirects only to the validated authorization endpoint. The callback rejects provider errors or missing parameters, consumes state before downstream network/crypto work, exchanges the code, retrieves JWKS, verifies the ID token and nonce, resolves only an explicit issuer/subject link, and then creates the existing secscan session cookie before redirecting to `/app`. Any downstream failure after state consumption cannot replay the transaction. Password login remains available and unchanged.
