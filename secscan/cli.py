...
def _add_history_db_argument(parser: argparse.ArgumentParser, *, default: Path | None) -> None:
    parser.add_argument("--history-db", type=Path, default=default, help="SQLite history database path")
...
def _add_scan_arguments(parser: argparse.ArgumentParser, target_help: str) -> None:
    ...
    parser.add_argument("--fail-on", default=None, choices=["NONE", "UNKNOWN", "LOW", "MEDIUM", "HIGH", "CRITICAL"], help="override the policy severity threshold")
    parser.add_argument("--policy", type=Path, help="YAML policy file containing thresholds, suppressions, and rules")
    parser.add_argument("--baseline", type=Path, help="previous secscan.json report used to classify findings")
    ...
    parser.add_argument("--no-history", action="store_true", help="do not record this completed scan")


def build_parser() -> argparse.ArgumentParser:
    registry = build_default_registry()
    parser = argparse.ArgumentParser(prog="secscan", description="Scan targets with normalized output")
    ...
    for capability in registry.capabilities():
        target_parser = scan_subparsers.add_parser(capability.name, help=capability.description)
        _add_scan_arguments(target_parser, capability.target_help)

    ecr_scan = scan_subparsers.add_parser("ecr", help="scan one approved ECR image by immutable digest")
    _add_scan_arguments(ecr_scan, "immutable ECR image URI from the inventory")
    ecr_scan.add_argument("--inventory", type=Path, required=True, help="ECR asset inventory JSON")
    ecr_scan.add_argument("--aws-config", type=Path, required=True, help="AWS discovery YAML config")
    ...
    finding_changes = subparsers.add_parser("finding-changes", help="compare the two latest finding-level scan records")
    ...
    finding_timing = subparsers.add_parser("finding-timing", help="summarize bounded finding observation episodes")
    ...
    finding_timing.add_argument("--limit", type=int, default=20, help="finding-enabled scans to include (2-100)")
    ...
    ecr.add_argument("--output", type=Path, default=Path("/reports/ecr-assets.json"), help="discovered asset inventory path")
    ec2 = discover_subparsers.add_parser("ec2", help="discover explicitly approved Amazon EC2 instances")
    ec2.add_argument("--config", type=Path, required=True, help="AWS discovery YAML config")
    ec2.add_argument("--output", type=Path, default=Path("/reports/ec2-assets.json"), help="discovered compute inventory path")
    ...
    ecr_batch = batch_subparsers.add_parser("ecr", help="scan up to 20 explicitly selected ECR image digests")
    ecr_batch.add_argument("--image-uri", action="append", required=True, help="exact immutable URI from the inventory; repeat for each image")
    ecr_batch.add_argument("--inventory", type=Path, required=True, help="ECR asset inventory JSON")
    ecr_batch.add_argument("--aws-config", type=Path, required=True, help="AWS discovery YAML config")
    ecr_batch.add_argument("--output-root", type=Path, default=Path("/reports/ecr-batch"))
    ecr_batch.add_argument("--fail-on", default=None, choices=["NONE", "UNKNOWN", "LOW", "MEDIUM", "HIGH", "CRITICAL"])
    ...
    sbom_inventory = inventory_subparsers.add_parser("sbom", help="normalize packages and declared licenses from an SBOM")
    sbom_inventory.add_argument("target", type=Path, help="CycloneDX or SPDX JSON SBOM")
    sbom_inventory.add_argument("--output", type=Path, default=Path("/reports/secscan.inventory.json"), help="versioned inventory JSON output path")
    ...
    inventory_compare = compare_subparsers.add_parser("inventory", help="compare two normalized SBOM inventories")
    inventory_compare.add_argument("baseline", type=Path)
    inventory_compare.add_argument("current", type=Path)
    inventory_compare.add_argument("--output", type=Path, default=Path("/reports/secscan.inventory.diff.json"))
    ...
    inventory_check = check_subparsers.add_parser("inventory", help="check declared licenses in a normalized inventory")
    inventory_check.add_argument("inventory", type=Path)
    inventory_check.add_argument("--policy", type=Path, required=True)
    inventory_check.add_argument("--output", type=Path, default=Path("/reports/secscan.inventory.policy.json"))
    return parser
...
def _print_history_entry(entry: ScanHistoryEntry) -> None:
    ...
    print(f"Severity: CRITICAL={entry.critical} HIGH={entry.high} MEDIUM={entry.medium} LOW={entry.low} UNKNOWN={entry.unknown}")
...
def _run_history(args: argparse.Namespace) -> int:
    ...
    for entry in entries:
        print(f"{entry.id:<3} {entry.created_at:<20} {entry.scanner:<12} {entry.critical:<8} {entry.high:<4} {entry.medium:<6} {entry.target}")
    return 0
...
def _build_trend(entries: list[ScanHistoryEntry], scanner: str, target: str) -> dict[str, object]:
    if len(entries) < 2:
        raise ValueError(f"at least 2 matching scans are required for scanner={scanner!r} target={target!r}")
    oldest = entries[0]
    latest = entries[-1]
    latest_counts = {severity: getattr(latest, severity) for severity in _TREND_SEVERITIES}
    changes = {severity: getattr(latest, severity) - getattr(oldest, severity) for severity in _TREND_SEVERITIES}
    series = [{"scan_id": entry.id, "created_at": entry.created_at, "duration_ms": entry.duration_ms, "severity": {severity: getattr(entry, severity) for severity in _TREND_SEVERITIES}} for entry in entries]
    return {"schema_version": 1, "scanner": scanner, "target": target, "scan_count": len(entries), "oldest_created_at": oldest.created_at, "latest_created_at": entries[-1].created_at, "latest": latest_counts, "change_since_oldest": changes, "series": series}
...
def _run_trends(args: argparse.Namespace) -> int:
    ...
    entries = HistoryStore(args.history_db).list_trend_scans(scanner=args.scanner, target=args.target, limit=args.limit)
    ...
    for entry in entries:
        print(f"{entry.id:<3} {entry.created_at:<20} {entry.critical:<8} {entry.high:<4} {entry.medium:<6} {entry.low:<3} {entry.unknown}")
    changes = trend["change_since_oldest"]
    assert isinstance(changes, dict)
    print("Change since oldest: " + " ".join(f"{severity.upper()}={changes[severity]:+d}" for severity in _TREND_SEVERITIES))
    return 0
...
def _build_finding_changes(observations: list[tuple[ScanHistoryEntry, tuple[StoredFinding, ...]]], scanner: str, target: str) -> dict[str, object]:
    if len(observations) != 2:
        raise ValueError(f"2 finding-level scans are required for scanner={scanner!r} target={target!r}")
    ...
    return {"schema_version": 1, "scanner": scanner, "target": target, "previous_scan": {"id": previous_scan.id, "created_at": previous_scan.created_at}, "current_scan": {"id": current_scan.id, "created_at": current_scan.created_at}, "summary": {"new": len(new_ids), "resolved": len(resolved_ids), "unchanged": len(unchanged_ids)}, "new": documents(new_ids, current), "resolved": documents(resolved_ids, previous), "unchanged": documents(unchanged_ids, current)}


def _run_finding_changes(args: argparse.Namespace) -> int:
    observations = HistoryStore(args.history_db).latest_finding_observations(scanner=args.scanner, target=args.target)
    ...
    if args.output:
        ...
        print(f"Finding changes: {args.scanner} {args.target} new={summary['new']} resolved={summary['resolved']} unchanged={summary['unchanged']}")
    return 0
...
def _episode_document(episode: _OpenFindingEpisode, resolved_by: ScanHistoryEntry | None) -> dict[str, object]:
    observed_seconds: int | None = None
    if resolved_by is not None and not episode.left_censored:
        observed_seconds = round((_timestamp(resolved_by.created_at) - _timestamp(episode.first_scan.created_at)).total_seconds())
        if observed_seconds < 0:
            raise ValueError("finding history timestamps must be chronological")
    return {"finding": asdict(episode.finding), "first_observed_scan": _scan_reference(episode.first_scan), "last_present_scan": _scan_reference(episode.last_scan), "resolved_by_scan": _scan_reference(resolved_by) if resolved_by else None, "left_censored": episode.left_censored, "observed_resolution_seconds": observed_seconds}


def _build_finding_timing(observations: list[tuple[ScanHistoryEntry, tuple[StoredFinding, ...]]], scanner: str, target: str) -> dict[str, object]:
    if len(observations) < 2:
        raise ValueError(f"at least 2 finding-level scans are required for scanner={scanner!r} target={target!r}")
    ...
    for observation_index, (scan, findings) in enumerate(observations):
        ...
        for fingerprint in sorted(current):
            if fingerprint in active:
                ...
                active[fingerprint] = _OpenFindingEpisode(finding=current[fingerprint], first_scan=scan, last_scan=scan, left_censored=observation_index == 0)
    open_episodes = [_episode_document(active[key], None) for key in sorted(active)]
    measurable = [value for episode in resolved if isinstance((value := episode["observed_resolution_seconds"]), int)]
    mean_seconds = round(sum(measurable) / len(measurable), 3) if measurable else None
    return {"schema_version": 1, "scanner": scanner, "target": target, "window": {"scan_count": len(observations), "oldest_scan": _scan_reference(observations[0][0]), "latest_scan": _scan_reference(observations[-1][0])}, "summary": {"resolved_episode_count": len(resolved), "open_episode_count": len(open_episodes), "left_censored_episode_count": sum(bool(episode["left_censored"]) for episode in [*resolved, *open_episodes]), "measurable_resolved_count": len(measurable), "mean_observed_resolution_seconds": mean_seconds}, "resolved_episodes": resolved, "open_episodes": open_episodes}


def _run_finding_timing(args: argparse.Namespace) -> int:
    if not 2 <= args.limit <= 100:
        raise ValueError("finding timing limit must be between 2 and 100")
    observations = HistoryStore(args.history_db).list_finding_observations(scanner=args.scanner, target=args.target, limit=args.limit)
    ...
    if args.output:
        ...
        assert isinstance(summary, dict)
        print(f"Finding timing: {args.scanner} {args.target} resolved={summary['resolved_episode_count']} open={summary['open_episode_count']} measurable={summary['measurable_resolved_count']} mean_observed_seconds={summary['mean_observed_resolution_seconds']}")
    return 0
...
def _run_scan(args: argparse.Namespace) -> int:
    ...
    request = ScanRequest(scanner_name=logical_scanner, target=args.target, timeout_seconds=args.timeout, output_dir=args.output_dir, environment=environment)
    ...
    report = build_report(args.target, list(result.findings), scanner_metadata, target_type=report_target_type)
    ...
    report["policy"] = {"fail_on": fail_on, "active_findings": len(evaluation.active_findings), "suppressed_findings": [{"vulnerability_id": suppressed.finding.vulnerability_id, "package_name": suppressed.finding.package_name, "reason": suppressed.reason, "expires": suppressed.expires.isoformat()} for suppressed in evaluation.suppressed_findings], "rule_matches": [{"rule": matched.rule_index, "vulnerability_id": matched.finding.vulnerability_id, "package_name": matched.finding.package_name, "fail_on": matched.fail_on, "reason": matched.reason} for matched in evaluation.rule_matches]}
    ...
    if not args.no_history:
        ...
        scan_id = HistoryStore(history_db).record_scan(scanner=logical_scanner, target=args.target, duration_ms=duration_ms, fail_on=fail_on, summary=report["summary"], report_path=report_path, sbom_path=sbom_path, diff_path=diff_path, secscan_version=_secscan_version(), scanner_version=str(scanner_metadata.get("version", "unknown")), findings=tuple(result.findings))
        print(f"History: recorded scan {scan_id} in {history_db}")
    print(json.dumps(report["summary"], sort_keys=True))
    print(f"Policy: fail_on={fail_on}, active={len(evaluation.active_findings)}, suppressed={len(evaluation.suppressed_findings)}, rules={len(evaluation.rule_matches)}")
    print(f"Artifacts written to {args.output_dir}")
    return 2 if policy_failed(list(evaluation.active_findings), fail_on, policy=policy) else 0
...
def _run_sbom_inventory(args: argparse.Namespace) -> int:
    ...
    assert isinstance(summary, dict)
    print(f"Inventory: packages={summary['package_count']} licensed={summary['packages_with_declared_license']}")
    print(f"Inventory written to {args.output}")
    return 0
...
def _run_ecr_batch(args: argparse.Namespace) -> int:
    ...
    for index, asset in enumerate(assets, start=1):
        ...
        scan_args = argparse.Namespace(target_type="ecr", target=asset.image_uri, inventory=args.inventory, aws_config=args.aws_config, output_dir=output_dir, fail_on=args.fail_on, policy=args.policy, baseline=None, timeout=args.timeout, history_db=history_db, no_history=args.no_history)
        try:
    ...
    if batch_exit_code == 0 and any(entry.exit_code == 2 for entry in entries):
        batch_exit_code = 2
    manifest = {"schema_version": 1, "image_count": len(entries), "exit_code": batch_exit_code, "entries": [asdict(entry) for entry in entries]}
