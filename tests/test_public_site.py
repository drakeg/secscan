...
def test_anonymous_visitors_can_browse_landing_login_and_registration(tmp_path: Path) -> None:
    ...
    assert "Compare plans" in landing.text
...
def test_authenticated_landing_replaces_signup_actions_with_account_actions(tmp_path: Path) -> None:
    client = TestClient(app_for(tmp_path / "jobs.db"))
    created = client.post(
    ...
    assert "Manage plan" in landing.text
    ...
    assert "href='/login'" not in landing.text
